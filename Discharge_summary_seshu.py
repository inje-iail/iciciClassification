...
def check_HOS(list_of_json, Headings):
    ...
    for i in range(len(list_of_json)):
        ...
        if i == 0:
            print(filepath)
            json_data = get_icr_data_from_image(filepath)
            if len(os.listdir(save_path)) >= 2:
                data = get_icr_data_from_image(os.path.join(save_path, str(1) + ".jpeg"))
                x = [*json_data, *data]
                json_data = x
            if len(os.listdir(save_path)) >= 3:
                data = get_icr_data_from_image(os.path.join(save_path, str(2) + ".jpeg"))
                x = [*json_data, *data]
                json_data = x
            if len(os.listdir(save_path)) >= 4:
                data = get_icr_data_from_image(os.path.join(save_path, str(3) + ".jpeg"))
                x = [*json_data, *data]
                json_data = x
            if len(os.listdir(save_path)) >= 5:
                data = get_icr_data_from_image(os.path.join(save_path, str(4) + ".jpeg"))
                x = [*json_data, *data]
                json_data = x
            if len(os.listdir(save_path)) >= 6:
                data = get_icr_data_from_image(os.path.join(save_path, str(5) + ".jpeg"))
                x = [*json_data, *data]
                json_data = x
            if len(os.listdir(save_path)) >= 7:
                data = get_icr_data_from_image(os.path.join(save_path, str(6) + ".jpeg"))
                x = [*json_data, *data]
                json_data = x
            break
...
save_path = r"D:\DS_work_Inje\discharge sum\CL-004846784"
