...
def dis_sum(list_of_json):
    ...
    for key in result.keys():
        ...
        if key_final.strip() in ["reg no", "registration no", "regno", 'reg no ','op number','opd no','reg  no', 'hospital no','op no', "mci regno",'hh no']:    ## 'admission no', 'umr no'
            final["registration no"] = result[key].replace("Name", "")
        elif key_final in ["admission date /time",'Admission', "adm date", "admission date",'d.o.a', 'admn dt',"date of admission",'admission date & time','date of admission',"doa", "admn date", "admitted on", "admdate", "admit date", "admission date/time", "admission datetime", "admitted","admission dt / tm","admitted date","admission dt"]:
            if "date of admission" not in final.keys():
                final["date of admission"] = result[key].replace(":", "").strip()
        elif key_final in ["discharge date / time",'Discharge', 'discharge dt','discharge date & time','discharge date','d.o.d', "dod",'date of discharge','date of procedure/ operation',"discharge dt","discharge dt ", "discharge date",   "discharge summary date", "discharged on", "discharged date", "discharge date/time", "discharge datetime", "disdate",'Date and Time of Discharge,' "date and time of discharge","discharge dt / tm","dischargeon","discharge date time"]: #"discharged"   ##  'discharge type',
        ...
        elif key_final in ["patient contact",'phone no ', 'contact no',"mobile", "mob no", "mobile no", "contact no", "mobile no/landline no", "phone no","phone",'tel number']:
            final["mobile"] = result[key]
        elif key_final in ["patient name", "name",'patient',"name", 'name of patient','patient/relative signature',"patient", "name of the patient",'head of department','name of the patient',"name ","|name of patient"]:  #'baby details',
            final["patient name"] = result[key].replace(":", "").strip()
        elif key_final in ["age", "age(as of today)", "age/gender", "age/sex", "gender/age", "gender / age","age/ sex","age / sex",'age / gender', 'age(as per today)']:
        ...
        elif key_final in ["mrd", "mrd#", 'mr no','mrdno',"mrd no", "mr no", "mr no / ip no",'mrdno ',"ipno",'mr number', 'umr no']: #'episode no'
            final["mrd"] = result[key].replace(":", "").strip()
        elif key_final in ["gender", "sex", "sox","sex "]:
            if "gender" not in final.keys():
                final["gender"] = result[key]
        elif key_final in ["bed", 'bed no','bed no ','ward / room/ bed', "ward/bed no", 'bed no', "war/room/bed", "bed no - ward", "ward / bed", "room no", "ward/bed", "bed details","bed category"]: #,'bed category',  ##  "ward/floor",  "ward no", "ward info", "ward", 'ssn no',
        ...
        elif key_final in ["ip no",'i p no',"patient id", "patient no", "in patient no", "patient no ", "patient identifier", "ipd no", "ipnc", "ipno", 'ipid']:
        ...
        elif key_final in ["history", 'case history',"medical history", "medical history of the patient","history of presenting illness","medical history & presenting complaints","presenting complaints",'maternal history ','social / family history','brief history']:
            final["history"] = result[key]
        elif key_final in ["medicine on admission", "medication on admission", "medications administered", "admission notes", "medication administered",'medications', "treatment given", "treatment given in ward", "medicines given in ward"]:
        ...
        elif key_final in ["follow up", 'followup','follow-up',"follow up on", "follow up date", "follow-up advice", "follow up advice", "next follow up", "follow up appointment","review","next appointment","follow up visit instructions","review","follow-up instructions","review on"]:
        ...
        elif key_final in ["discharge recommendations", "recommendations",'review','recommedations','preventive plan of','preventive plan of care','when to obtain urgent care']:
